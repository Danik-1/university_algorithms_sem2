...
class ListGraph {
 public:
  ...
  std::vector<std::pair<VType, size_t>> GetNeighbours(const VType& vertex) {
...
void BridgesDfs(ListGraph<VType>& graph, const VType& vertex,
                const VType& prev_vert, std::vector<Counter>& time_in,
                std::vector<Counter>& ret, const Counter& time,
                std::vector<size_t>& bridges) {
  ret[vertex] = time_in[vertex] = time;
  for (auto& v : graph.GetNeighbours(vertex)) {
    VType next_vert = v.first;
    if (next_vert == prev_vert) {
      continue;
    }
